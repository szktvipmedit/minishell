...
static char *ft_search_cmd_from_path(char **paths, char *cmd, t_shell *shell)
{
	...
	if (access(cmd, X_OK) == 0)//絶対パスが与えられた場合はここでそのままreturnする。./minishellもここで実行可能になる。
		return (cmd);
	i = 0;
	while(paths[i] != NULL)
	{
...
void	ft_execute_command(char **args, char **environ_list_head, t_shell *shell)
{
	...
	if (execve(cmd_path, args, environ_list_head) == -1)//以下、execveが失敗した場合。
	{
		free(cmd_path);
		command = ft_strdup(args[0]);//args[0]がNULLでないことは保証されてる。args[0]が下でfreeされる前にmallocしておく
		if (command == NULL)
			ft_free_all_and_exit(shell, 1);
