...
void	ft_prepare_redirects(t_node	node, t_shell *shell)//infileとoutfileが交互に来るパターンに対応した。
{
	size_t	i;
	...
	while (i < node.redirects_count)
	{
		if (node.redirects[i].type == INPUTFILE || node.redirects[i].type == DELIMITER)
			ft_get_infile(node, node.redirects[i], shell);//int型のreturn値が返ってくるが使わない。
		if (node.redirects[i].type == OUTPUTFILE || node.redirects[i].type == APPENDFILE)
			ft_get_outfile(node.redirects[i], shell);
		i++;
	}
}
