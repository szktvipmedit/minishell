...
static void	ft_change_token_type(t_token **token)
{
	...
	while (tmp != NULL)
	{
		if (tmp->type == REDIRECT)//ft_check_syntaxを通しているため、REDIRECT直後のtokenはWORDであることが保証されてる。
		{
			if (ft_strncmp(tmp->word, "<<", 2) == 0)//まず"<<",次に"<"の順で分岐させる必要がある。
...
void	ft_parse(t_token *token, t_shell *shell)
{
	...
	ft_change_token_type(&token);//リダイレクト記号の直後のtoken_typeをwordからDELIMITERなどに変更する
