...
static void	ft_specify_fd_error(char **line, t_shell *shell)//今回は実装しない
{
	ft_putstr_fd("Sorry, minishell does not support `[n]<', `[n]>', `[n]<<', `[n]>>'.\n", 2);
	while (**line != '\0')
		(*line)++;//エラーが見つかったので、残りのlineはtokenizeせずに飛ばす。
	shell->tokenize_error = -1;//shell構造体は初期値０だから、それを-１に変える。
}

static void	ft_force_overwrite_error(char **line, t_shell *shell)//今回は実装しない
{
...
static void	ft_double_pipe_error(char **line, t_shell *shell)//今回は実装しない
{
	ft_putstr_fd("Sorry, minishell does not support `||'.\n", 2);
	while (**line != '\0')
		(*line)++;
